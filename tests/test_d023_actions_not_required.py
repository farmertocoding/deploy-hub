...
def test_phase_gate_is_local_make_conformance_3_not_a_gha_check():
    ...
    phase6 = _recipe("conformance-6")
    assert phase6 is not None, "conformance-6 has no recipe"
    assert "--phase 6" in phase6, phase6
    assert "--exclude-tier t2" in phase6, (
        f"conformance-6 must omit t2 so it does not demand docker:\n{phase6}"
    )
    assert "--exclude-tier t3" in phase6, (
        f"conformance-6 must omit t3 so it does not demand Multipass:\n"
        f"{phase6}"
    )
