# WAIVERS.md — one line per waived finding: fingerprint + reason + date
# (build-process.md §4: a finding is a fix-with-regression-test OR a line here. Nothing else.)

WAIVED: .github/workflows+supply-chain+actions-pinned-by-tag — third-party actions pinned by major tag, not SHA; repo is local-only until D-001 GitHub move (Phase 2.5 at latest), at which point actions get SHA-pinned in the same PR that enables branch protection (2026-08-03)

# R4-9 (SPEC-gate-integrity.md §3.2 rule 4): `verify: checklist` reqs now have to name the
# gate that enforces them. These two have no gate on this tree. Per the spec, a waiver is
# the correct answer where no gate exists yet — inventing one to reach green is not. Each
# names what would retire the waiver. Fingerprint = req id + uncovered(no gate).
# RETIRED 2026-08-23 (Phase 4 Task 8): SEC-B4-REDIS-CROWN-JEWEL
# — tests/test_redis_crown_jewel.py parses docker-compose.yml (redis unpublished
# + requirepass) and pins CELERY_*_SERIALIZER json; verify: test. C9: compose
# parse, not an external port-scan.
WAIVED: PROC-REGRESSION-TEST — uncovered(no gate): build-process.md §5 puts this on the auto-fix PR template plus "a path-glob check on any PR closing an issue"; neither the template nor the check exists on this tree, and a PR-scoped check cannot run until the repo moves to GitHub (D-001). Retire with the PR-template + path-glob workflow in the D-001 move PR (2026-08-11)
WAIVED: PROC-SENSITIVE-HUMAN-MERGE — uncovered(no gate): the `sensitive-path-guard` job in push-checks.yml is a placeholder that only echoes "wire branch-protection + CODEOWNERS when repo moves to GitHub (D-001)"; naming it in `gate:` would make check.py green on a step that enforces nothing, which is the exact defect R4-9 exists to kill. Retire when CODEOWNERS + branch protection derived from conformance/paths.yaml land with D-001 (2026-08-11)

# Round-5 F1: a gate that enforces *something else* is not this requirement's gate. The
# review found SEC-69-NO-SECRETS-IN-EXHAUST reported `verified` on `make log-scrub`, which
# greps source files for `SECRET_KEY\s*=`; the requirement is about exhaust after write and
# names a CI scrubber over captured test output. `gate: log-scrub` removed from the registry;
# log-scrub itself kept as the source-scan gate it genuinely is.
# RETIRED 2026-08-23 (Phase 4 Task 8): SEC-69-NO-SECRETS-IN-EXHAUST
# — tests/test_secrets_in_exhaust.py + pytest plugin (scripts_dev/exhaust.py)
# over captured stdout/stderr/log and Celery kwargs; verify: test. C9: captured
# exhaust, not make log-scrub.

# Round-5 F9: an unreachable check, recorded rather than left silent. Fingerprint = the
# artifact tree + why nothing reads it.
# R4-11 WI-3 (audit finding, 2026-08-11): a requirement read `verified` on one clause of
# three. Fingerprint = req id + the unproven clauses. The two markers that produced the
# green were removed rather than left claiming the whole text; the tests they were on
# still run. Retiring this waiver means restoring those markers. See DECISIONS.md D-009.
# Clause (b) retired 2026-08-11 by D-010 / SPEC-django-common-checks.md.
WAIVED: SCAN-M4-EXPOSURE-AUTH — blocker-escalation clause unimplemented: "Blocker when the wizard tags financial/personal data" has no code path — `scanner/modules/fallbacks.py::_check_exposure_auth` returns only ok/warning, and no wizard question anywhere tags data sensitivity (question set unchanged: site.domain, site.exposure, django.{domain,exposure,db,env.*}, node-ts.{service-package,dev-packages,data-dir,worker-threads,exposure,exclusive-upstream}, dockerfile.{domain,exposure,port,env}, static.{domain,exposure}), so there is no answer for an escalation to read. Clause (b) of the 2026-08-11 waiver — django scan reports never carried `core.exposure-auth` — is RETIRED (2026-08-11): `scanner/core.py::scan` now composes the common core suite into every report (D-010, Architect ruling; the gap was the whole seven-check suite, not this check alone). The warning clause now holds for every registered module, proven by tests/test_scanner_fallbacks.py::test_no_auth_indicators_fires_exposure_auth_warning, ::test_auth_indicators_silence_exposure_auth, tests/test_scanner_node_ts.py::test_issue_r4_11_a_node_ts_scan_surfaces_the_exposure_auth_check and tests/test_scanner_django.py::test_a_django_scan_surfaces_the_exposure_auth_check (all unmarked while this waiver stands — a marked test would claim the requirement's whole text and the escalation clause remains unbuilt). Retire the remainder when a wizard data-sensitivity question exists and public-exposure + financial/personal + no-auth escalates `core.exposure-auth` to blocker with a marked test — at which point the SCAN-M4-EXPOSURE-AUTH markers go back on. Original waiver and clause-(b) retirement both dated (2026-08-11)

WAIVED: conformance/demos/phase-1+unchecked-by-any-requirement — no phase-1 requirement is `verify: demo`, so the four artifacts under conformance/demos/phase-1/ are read by nothing: check.py only opens the paths a `demo:` key names (or the phase-N.md fallback), and the sole demo req is P0-WS-DEMO at phase 0. This is why R4-10 (the stale E-invoice scan record) did not surface when SPEC-gate-integrity.md §3.4 predicted it would — the content check landed, but no requirement points it at that tree. Retire by adding a phase-1 `verify: demo` requirement whose `demo:` names conformance/demos/phase-1/ in the registry PR (2026-08-11)

# Phase-1 demo follow-up 5 (2026-08-20): pointing the scanner at this repo is not
# a customer scan. The findings are the detector's own examples and the suite's
# fixtures. D-012, which would have labelled test material, is parked.
WAIVED: core.secret-scan+self-scan-detector-vectors — a scan of the Hub tree flags published-format examples in `scanner/modules/fallbacks.py` comments (`postgres://user:REALPASSWORD@host`) and fixture values under `tests/` and `scripts_dev/`. Those are the detector and its proofs, not leaked credentials. Phase 1's surface is customer django/node trees. Retire with D-012's return (declared test material) or a Hub-tree exclusion (2026-08-20)

# spec-mutation-gate.md §4: the mutation gate's ONLY escape hatch. There is no baseline
# file and no allowlist — a surviving mutant is a finding, fixed with a failing-first
# test, or it is one line here with its own justification. Fingerprint =
# `mutation+<file>+<mutant id>`, exactly as the gate prints it, so `mutmut show <id>`
# reproduces the diff being waived in one step.
#
# EVERY LINE BELOW CLAIMS THE SAME KIND OF THING and it is a strong claim: the mutant is
# EQUIVALENT — a different spelling of identical behaviour, so no test anywhere can
# distinguish it, and writing one would be writing a test that asserts nothing. That is
# the one honest reason to waive a survivor; "hard to test" is not, and none of these
# say it. The gate fails if any of these mutants stops surviving, so a waiver cannot
# outlive the code it excuses.
#
# THREE OF THEM WERE ALSO FINDINGS ABOUT THE CODE, recorded rather than fixed by the
# gate PR itself (deleting production code inside the PR that adds the gate judging it
# is what §6 forbids). TWO ARE NOW RETIRED, in round 9, and their lines are gone with
# them: `missing_required`'s unread first parameter and the unread `code` half of
# `validate_answers`' error entries are both deleted, so
# `wizard.materialize.x_preflight__mutmut_103`,
# `wizard.questions.x_validate_answers__mutmut_10` and `_11` no longer exist as mutants
# at all — which is the shape a retirement should take, since the gate FAILS on a waiver
# whose mutant stopped surviving.
#
# The third stands: `_read_entry`'s `normalized.startswith("/")` arm cannot fire on a
# platform where `os.sep == "/"` because `PurePosixPath.is_absolute()` already covers
# it. It is left because the arm is defensive on a platform this fleet does not deploy
# to, and deleting a guard for a case nobody here can test is a wider decision than a
# waiver retirement.
WAIVED: mutation+scanner/declarations.py+scanner.declarations.x__read_entry__mutmut_56 — equivalent mutant: `is_absolute() and startswith("/")` collapses to `is_absolute()` wherever `os.sep == "/"`, because `posix` is built from `normalized.replace(os.sep, "/")` and `PurePosixPath(s).is_absolute()` holds exactly when `s` starts with `/`. The `or ":" in parts[0]` arm is untouched and is pinned by test_each_path_refusal_says_which_rule_refused_it. Retire it by deleting the redundant arm (a round-9 finding, not a gate PR's edit), which removes the mutant with it (2026-08-16)
WAIVED: mutation+scanner/declarations.py+scanner.declarations.x__read_entry__mutmut_58 — equivalent mutant, same proof as mutmut_56: `startswith("XX/XX")` is never true, and the arm it disables is already subsumed by `posix.is_absolute()` on every platform this runs on. `/etc/secrets` is still refused as absolute, by the first arm, and that refusal is asserted (2026-08-16)
WAIVED: mutation+scanner/declarations.py+scanner.declarations.x_confirm_question_id__mutmut_19 — equivalent mutant: `"utf-8"` -> `"UTF-8"`. Python's codec lookup is case-insensitive and normalizes both to the same codec, so the digest bytes are identical. No test can tell them apart because there is nothing to tell apart (2026-08-16)
WAIVED: mutation+scanner/declarations.py+scanner.declarations.x_confirm_questions__mutmut_16 — equivalent mutant: dropping `default=None` from the `WizardQuestion(...)` call leaves the dataclass field's own default, which is `None` (scanner/core.py). R7-11's point — that an unanswered claim is not an accepted one — is unaffected, and the test that pins it stays green because the value is unchanged. Accidentally dropped from this file in bae4920 while adding PART-U1; restored because the mutant still survives. Retire it by making the field have no default, which is a scanner/core.py decision, not this gate's (2026-09-02)
WAIVED: mutation+wizard/materialize.py+wizard.materialize.x__apply_answers__mutmut_19 — equivalent mutant: `.decode("utf-8")` -> `.decode("UTF-8")` on the vault plaintext, same case-insensitive codec lookup. Was `__mutmut_17` until R9-SEC-1 added two mutation points earlier in the same function; mutmut numbers a function's mutants in source order, so a waiver by id survives only until the function above it changes. The gate caught the stale id itself — it reported `__mutmut_17` as `killed`, which is a spent waiver — which is the property that makes renumbering safe to do by hand. First recorded 2026-08-16, renumbered on the date below (2026-08-17)
WAIVED: PART-U1-NAMED-PARTNER — uncovered: named-partner evidence is operator-owned and must not be synthesized; deferred until a real PART-U1 artifact is committed. T1 fixture slugs are not this proof. Retire by adding the real named-partner demo, never a generated stand-in (2026-08-28)
WAIVED: mutation+hub/renderers.py+hub.renderers.xǁContainedJSONRendererǁrender__mutmut_17 — equivalent mutant: `ensure_ascii=self.ensure_ascii` -> `ensure_ascii=None`. `self.ensure_ascii` is DRF's `JSONRenderer.ensure_ascii = not api_settings.UNICODE_JSON`, a CLASS attribute evaluated once at import to `False` on this settings, and `json.dumps` treats `ensure_ascii=None` identically to `ensure_ascii=False` (None is falsy). Both spellings therefore render byte-for-byte the same on every code point — CJK stays literal, the CONTROL_CLASS is escaped by `json_safe` regardless — so no test can separate them. Not equivalent IN PRINCIPLE: were an operator to set `UNICODE_JSON=False`, `self.ensure_ascii` would be `True` and the two would diverge — but the attribute is frozen at import and no `override_settings` reaches it after start, so the difference is unobservable from inside the suite (the same shape as the `x_load__mutmut_19` locale waiver above). Retire it by reading the setting live (`not api_settings.UNICODE_JSON`) if that flag ever needs to be honoured at runtime, at which point a `@override_settings` test kills it (2026-08-18)
WAIVED: mutation+core/rbac.py+core.rbac.x_is_system_admin__mutmut_6 — equivalent mutant: `getattr(user, "is_authenticated", False)` -> `None`. Django User/AnonymousUser always define the attribute, so the default is never read; when it is missing, `bool(user and None and is_superuser)` is False, same as False. Retire by dropping the default (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_workspace_payload__mutmut_6 — equivalent mutant: same unused `is_authenticated` default False->None on Django user objects (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_workspace_payload__mutmut_9 — equivalent mutant: trailing-comma default elided; `getattr(user, "is_authenticated")` still hits the real attribute on every Django user (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_workspace_payload__mutmut_12 — equivalent mutant: default True is unread because `is_authenticated` is always present on the user objects this function receives (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_resolve_workspace__mutmut_6 — equivalent mutant: unused `is_authenticated` default False->None (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_resolve_workspace__mutmut_9 — equivalent mutant: unused `is_authenticated` default elided (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_resolve_workspace__mutmut_12 — equivalent mutant: unused `is_authenticated` default True (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x__hud_capabilities__mutmut_10 — equivalent mutant: `getattr(settings, "HUD_UI_V1", True)` -> None. `HUD_UI_V1` is assigned in hub/settings/base.py, so the default is never read (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x__hud_capabilities__mutmut_13 — equivalent mutant: HUD_UI_V1 default elided; setting always present (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x__hud_capabilities__mutmut_16 — equivalent mutant: HUD_UI_V1 default True->False unread (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_hud_ui_enabled__mutmut_4 — equivalent mutant: HUD_UI_V1 default True->None unread (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_hud_ui_enabled__mutmut_7 — equivalent mutant: HUD_UI_V1 default elided unread (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_hud_ui_enabled__mutmut_10 — equivalent mutant: HUD_UI_V1 default True->False unread (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_request_workspace__mutmut_12 — equivalent mutant: `headers.get("X-Workspace")` -> `"x-workspace"`. Django HttpHeaders is case-insensitive, so both spellings resolve the same header (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_request_workspace__mutmut_13 — equivalent mutant: `"X-Workspace"` -> `"X-WORKSPACE"`, same case-insensitive header map (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_request_workspace__mutmut_16 — equivalent mutant: `"X-Workspace-Id"` -> `"x-workspace-id"`, same case-insensitive header map (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_request_workspace__mutmut_17 — equivalent mutant: `"X-Workspace-Id"` -> `"X-WORKSPACE-ID"`, same case-insensitive header map (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_capabilities_for__mutmut_8 — equivalent mutant: `getattr(membership, "is_bootstrap", False)` -> `getattr(None, ...)`. The default is False, so a persisted membership (no is_bootstrap) still takes the real-role path (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_capabilities_for__mutmut_10 — equivalent mutant: is_bootstrap default False->None; missing attr is still falsy (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_capabilities_for__mutmut_14 — equivalent mutant: `"is_bootstrap"` -> `"XXis_bootstrapXX"`; getattr misses and returns False, same as a row without the flag (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_capabilities_for__mutmut_15 — equivalent mutant: `"is_bootstrap"` -> `"IS_BOOTSTRAP"`, same missing-attr False (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_user_role__mutmut_11 — equivalent mutant: getattr(None, "is_bootstrap", False) on a real membership is False, same as no bootstrap flag (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_user_role__mutmut_13 — equivalent mutant: is_bootstrap default False->None, falsy either way (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_user_role__mutmut_17 — equivalent mutant: `"is_bootstrap"` -> `"XXis_bootstrapXX"`, missing attr False (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_user_role__mutmut_18 — equivalent mutant: `"is_bootstrap"` -> `"IS_BOOTSTRAP"`, missing attr False (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireRecentTouchǁhas_permission__mutmut_6 — equivalent mutant: `getattr(request, "user", None)` default elided; DRF/Django always set request.user before has_permission (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireSystemAdminǁhas_permission__mutmut_6 — equivalent mutant: same unread getattr default on request.user (2026-08-28)
WAIVED: mutation+core/findings.py+core.findings.x__transition__mutmut_9 — equivalent mutant: `workspace=None` passed to audit(); `_resolve_workspace` infers the finding's workspace from `row`, so the stored workspace_id is unchanged (2026-08-28)
WAIVED: mutation+core/findings.py+core.findings.x__transition__mutmut_17 — equivalent mutant: omitting `workspace=row.workspace` from audit(); same inference from the Finding object (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireActionǁhas_permission__mutmut_12 — equivalent mutant: `or ""` -> `or "XXXX"` when action_id is missing; neither string is in ACTION_CAPABILITY, so both fail closed (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireActionǁhas_permission__mutmut_9 — equivalent mutant: getattr default elided on view.action_id; missing attribute is still falsy and fails closed (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_has_operator_capability__mutmut_3 — equivalent mutant: `resolve_workspace(None)` inside has_operator_capability; `user_capabilities` re-resolves from `user` when workspace is None, so the capability check is unchanged (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_resolve_workspace__mutmut_34 — equivalent mutant: `select_related(None)` still returns the membership row; only query count changes (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_workspace_payload__mutmut_19 — equivalent mutant: same unread select_related(None) (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_workspace_payload__mutmut_18 — equivalent mutant: `order_by("workspace__slug", "id")` -> `order_by("workspace__slug")`; memberships are unique per workspace so the extra id key never breaks ties (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_resolve_workspace__mutmut_33 — equivalent mutant: same unused id tie-break on unique (user, workspace) memberships (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireRecentTouchǁhas_permission__mutmut_47 — equivalent mutant: `make_aware(touched, None)` uses Django's default timezone; hub.settings.dev TIME_ZONE is UTC, the same tz the code passes explicitly (2026-08-28)
WAIVED: mutation+core/permissions.py+core.permissions.xǁRequireRecentTouchǁhas_permission__mutmut_49 — equivalent mutant: `make_aware(touched,)` drops the explicit tz and uses the default UTC timezone (2026-08-28)
WAIVED: mutation+core/rbac.py+core.rbac.x_user_role__mutmut_22 — equivalent mutant: `return ""` -> `"XXXX"` on the bootstrap-and-not-staff branch. `workspace_membership` only builds a bootstrap object for staff/superuser, so that return is unreachable (2026-08-28)

# Phase 2 round-1 leftovers the controller ruled in-bounds or Phase-2.5/3.
# Fingerprints match `.superpowers/sdd/phase-2-tasks/round-1-*.md`.
WAIVED: tests.test_crash_kill_matrix+REL-P3+t1-runtimeerror-not-t2-crash — HUB_TEST_CRASH_AFTER_STEP as in-process RuntimeError is the Phase 2 kill-matrix; SIGKILL of a live worker is Phase 2.5 harness. The T1 1–9 matrix stays. (2026-08-21)
WAIVED: tests.acceptance.test_phase_2+Q4-transcription+hub-test-target-is-t1 — T1 FakeTransport acceptance is allowed; hub-test-target is the T2 demo record. Retire when the acceptance clause is the T2/T3 nodeid. (2026-08-21)
# RETIRED 2026-08-23 (leftover Task 7): tests.test_pipeline_sample_node_site+PIPE-S4-READINESS-GATE+t2-instant-ready-stub
# — host-built sample-node-site image docker-load into hub-test-target;
# tests/test_pipeline_sample_node_site.py::test_t2_real_node_image_builds_on_vfs
# deploys Fastify /healthz (backfill_pct, not alpine T2_SERVE_PY).
# RETIRED 2026-08-22 (Task 13): frontend/src/App.jsx+UX-F8-SIMULATION-STATES+demo-pane-no-site-deploy-topics
# — frontend/tests/simulation-states.test.ts::every_new_state_renders_in_simulation_mode
# paints site/deploy/finding/map states on product screens, not the demo pane.
# RETIRED 2026-08-22 (Task 13): simulation/seed_v0.json+review3-N2+warming-omits-elapsed-expected
# — seed_v1 warming carries elapsed_s/expected_s and SiteObserved renders
# "45s elapsed / 600s expected".
# RETIRED 2026-08-22 (Task 16): deploys/breakglass.py+SEC-P5+runbook-lacks-impact-and-dns
# — tests/test_breakglass.py now asserts impact before commands, Hub-side
# upsert_record only, freshness metadata, 0400 root-owned, no token/secret,
# and the alert-protocol §3 advisory-only note.
WAIVED: REL-P2-HUB-DOWN-SITES-UP+verify-demo+24h-unproven — demo record is T2-now and honest; the hub-down prober is now a real external HTTP GET (D-042); the 24 h form is a dated calendar item, not a claimed duration. check.py verified file presence, not duration. First recorded 2026-08-21. (2026-08-22)

# Phase 2.5 exit (Task 16, amended by the panel fix wave). D-024: a `tier: t3` req
# is verified only by a passed @pytest.mark.t3 test, and on a host that cannot run
# one the honest state is skipped-only plus a dated line here — never a T1-sibling
# green. The two host-without-multipass lines (HARNESS-T3-NIGHTLY,
# HARNESS-T3-UFW-TRUTH) were RETIRED 2026-08-22: Multipass 1.16.3 was installed on
# this host and the first live T3 run is green (16 passed, 3 credential skips,
# reaper-empty teardown) — recorded in conformance/demos/phase-2.5/t3-run.txt.
# The self-refusing probes (tests/harness/multipass.py::waiver_illegal_if plus
# tests/acceptance/test_phase_2_5.py::test_t3_skip_cannot_verify_tier_t3) that
# forced that retirement stay armed for any future host-without-multipass line.
WAIVED: HARNESS-T3-LE-STAGING — no-test-zone-credentials: HUB_TEST_CF_TOKEN is not set on this host and no purpose=test DnsZone is enrolled, so tests/test_t3_https.py is skipped-only per the req's own text and D-031 — HTTPS + LE staging + wss-through-CF run only when the test-zone token is present, and absent credentials must never become a T1-sibling green. Multipass itself is present and live since 2026-08-22 (conformance/demos/phase-2.5/t3-run.txt); only the Cloudflare test-zone token is missing. Retire with the first credentialed run against the Cloudflare TEST zone (2026-08-22)
WAIVED: DNS-CF-T3-LIVE — no-test-zone-credentials: HUB_TEST_CF_TOKEN is not set on this host, so tests/test_t3_cf_live.py is skipped-only per the req's own text and D-043 — the product adapter's live upsert against the allowlisted purpose=test zone runs only when the test-zone token is present, and absent credentials must never become a T1-sibling green. Retire with the first credentialed run, in the change that records it under conformance/demos/phase-3/ (2026-08-22)

# Phase 3 exit (Task 19). Clause-scoped full-text ids (SCAN-M4 shape, D-035 /
# D-040): the buildable clauses are marked on their split ids; these two sit
# uncovered because a marker would claim the unbuilt clause. REL-P2 24 h and
# the D-022 checklist lines stay above. D-025 alpine PIPE-S4 is RETIRED
# (host-load). UX-F8 and SEC-P5 stay RETIRED. Do not read a
# MON-C7 or MAP-96 line here — both landed.
# RETIRED 2026-08-23 (Phase 3b Task 9): PROV-J7-COMPOSE-AWARE-ADOPT
# — tests/test_adopt_compose.py + tests/test_adopt_flow.py +
# tests/acceptance/test_phase_3_5.py::test_compose_plan_is_one_v5_site
# (T1 fakes; classify three fixture stacks to one V5 Site).
# RETIRED 2026-08-23 (Phase 3b Task 9): PROV-E6-ADOPT-TEMP-SUBDOMAIN
# — tests/test_adopt_flow.py + tests/test_adopt_reaper.py +
# tests/acceptance/test_phase_3_5.py::test_temp_subdomain_on_site_zone_cleans_up
# (T1 FakeDnsProvider; temp on Site.dns_zone; cleanup + 24 h reaper).
# RETIRED 2026-08-26 (Phase 7.4 Task 2): SEC-B2-NO-DNS-TOKENS-ON-TARGETS
# — both clauses hold: no token on target (SEC-B2-NO-TOKEN-ON-TARGET) and
# Hub-central DNS-01 for unproxied sites (TLS-B2-HUB-DNS01-UNPROXIED).
# Full-text SEC-B2 marker restored on the issue NAMED.
# RETIRED 2026-08-23 (Phase 4 Task 2): UX-F5-ACTION-TIERS
# — SEC-F5-T1-HARDWARE-TOUCH marked; full-text UX-F5 markers restored.
# T1 is WebAuthn hardware touch + type-the-name; TOTP never writes
# hardware_touch_at; T3 never takes RequireRecentTouch (D-062).

# Phase 4 Task 0 (D-054 / D-055 / D-060). Do not retire REL-P2, LE-staging,
# or DNS-CF-T3-LIVE.
# RETIRED 2026-08-26 (Phase 7.4 Task 2): TLS-B2-HUB-DNS01-UNPROXIED
# — Hub-central DNS-01 T1 Fake + Beat renew marked on issue/refuse/renew
# proofs. Live Let's Encrypt stays skipped (HARNESS-T3-LE-STAGING).
# SCAN-DECLARED-TEST-MATERIAL and SCAN-DECLARED-GUARDS: Task 3 retired the
# clause-scoped full-text waivers. Markers live on tests/test_d012_reland.py.

# Phase 4 Task 9 (D-058 / D-043). Live Tailscale poll is skip-unless-configured.
# T1 FakeTailscale proves unknown-device. Do not invent a live token env.
WAIVED: SEC-B8-TAILSCALE-DEVICE-POLL — skip-unless-configured: HUB_TAILSCALE_API_TOKEN_REF defaults empty; the live device-list poll writes CheckRun.Kind.TAILSCALE_DEVICES SKIPPED and never SUCCEEDED. T1 FakeTailscale covers unknown-device Finding. Do not invent a live token env (D-058 / D-043). Retire when a vault ref is present on the host of record. (2026-08-23)
