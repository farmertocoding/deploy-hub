...
WAIVED: mutation+scanner/declarations.py+scanner.declarations.x_load__mutmut_21 — equivalent mutant: `path.read_text(encoding="utf-8")` -> `"UTF-8"`. Same case-insensitive codec lookup as `x_confirm_question_id__mutmut_19`; the file bytes decode identically. Was `__mutmut_19` until `confirm_questions` gained a mutation point and renumbered `load`'s later mutants. No test can tell the spellings apart (2026-09-11)
...
WAIVED: provision/backup.py+hub-off-box-pg-dump — uncovered: off-box Hub pg_dump/restore target is operator-owned live storage (bucket, host, credentials). This repo forbids synthesizing live cloud credentials or invented backup endpoints (same class as PART-U1 / HUB_TEST_*). Not a requirements.yaml id (path-shaped so the registry check does not treat it as a typo'd req). Retire by committing a real backup destination and a restore drill that pg_restores into a clean instance (2026-09-11)
