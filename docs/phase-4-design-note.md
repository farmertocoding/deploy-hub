...
**Revision:** r1 — Architect draft plus five-seat scope panel APPROVE-WITH-CHANGES (Architect / Security / QE / SRE / UX). Orchestrator adjudication is in `.superpowers/sdd/phase-4-tasks/progress.md`. §7 is binding.
...
**Closed schema wave:** `0009_phase3.py` and `0010_phase3b.py` stay closed. Phase 4 adds **one** migration, `0011_phase4.py`, on **`DnsZone` + `AuditEvent` only** — denormalized `DnsZone.provider` + unique `(provider, name)`; `AuditEvent.prev_hash` (and `shipped_at` if the T1 fake shipper lands in the same wave). No new Hub tables. Backup artifacts reuse `CheckRun.Kind.BACKUP` with a closed results schema unless listing is impossible, in which case the panel must record a second-wave exception. Do not add a Site FK to `CheckRun`. Do not add `Site.tier`. django-otp-webauthn’s own migrations are third-party and allowed beside `0011`, not a second Hub schema wave.
...
9. **KMS KEK rung ③ adapter, T1 only, plus DEK cache.** `vault/kek.py` grows `KmsKEK`. `get_backend()` accepts `VAULT_KEK_BACKEND=kms` and **refuses unless configured**. Tests use moto 5.x `@mock_aws` (**not LocalStack**). Keyfile stays the test/dev rung. `vault/service.py` caches unwrapped DEKs in process memory (keyed by secret pk + kek_id; never log/serialize); unwrap on miss only. A KMS blip must not fail every `get` and must not take down serving sites. `rewrap` is the ①→③ path (already written). **Live AWS KMS is a Joseph interrupt.** Prod settings must not default to `kms` in this wave.
...
**Migration `0011_phase4.py` (DnsZone + AuditEvent only):**
- `DnsZone.provider` CharField, populated from `account.provider`, `UniqueConstraint(fields=["provider", "name"], name="uniq_dnszone_provider_name")`. Keep `uniq_dnszone_account_name`. `clean()` stays defense-in-depth.
- `AuditEvent.prev_hash` CharField(max_length=64, blank=True, default=""). `core/audit.py::audit()` writes `hash(prev_hash + canonical_row)`. Genesis row uses empty prev.

No new Hub models. WebAuthn device tables come from `django-otp-webauthn`’s migrations.

`CheckRun.Kind` Python choices (CharField, no migration): `SSH_ROTATE = "ssh_rotate"`, `BACKUP = "backup"`, `ATTACK_PLAYBOOK = "attack_playbook"`, `TAILSCALE_DEVICES = "tailscale_devices"`. Adopt results closed schema unchanged.
...
- **D-056** One schema wave: `0011_phase4.py` = `DnsZone.provider` + unique `(provider, name)` + `AuditEvent.prev_hash`. No new Hub tables. CheckRun kinds are Python-only. django-otp-webauthn migrations are third-party.
...
- **D-059** `KmsKEK` T1 (moto) + refuse-unless-configured + in-process DEK cache. Keyfile remains test/dev. Live AWS KMS is a Joseph interrupt (~$1/mo). YubiKey rung ② may slip.
...
- KMS: `VAULT_KEK_BACKEND=kms`; refuse if missing key id. Tests use moto.
- Audit ship (SLIP): skip unless bucket configured.

### C7 — Backup

Persist + Beat + P1. List + test-now on **Sites detail**. Restore command block is text. No `POST .../restore/`. Fingerprint for backup failure is existing `hub-db-or-backup-failure`. Restore-drill SKIPPED only when siteless.
...
`0011` is DnsZone.provider + unique `(provider, name)` + AuditEvent.prev_hash. CheckRun kinds Python-only. `conformance-4` excludes t2/t3. `conformance-3` all-tiers Phase 3. New phase-4 ids are tier-less. Task 0 waives DNS-01 and SCAN-M4-splits D-012.
